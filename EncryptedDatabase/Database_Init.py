...
def create_tables():
    ...
    cursor.execute('CREATE TABLE METADATA (id INT PRIMARY KEY AUTO_INCREMENT, filename VARCHAR(50) UNIQUE, '
                   'filetype VARCHAR(50), file_location VARCHAR(255)  UNIQUE, size FLOAT, attributes INT UNSIGNED, '
                   'owner_uid INT UNSIGNED, owner_gid INT UNSIGNED, mode VARCHAR(50), '
                   'date_created TIMESTAMP, date_modified TIMESTAMP, date_accessed TIMESTAMP, encryption_id INT NOT NULL, '
